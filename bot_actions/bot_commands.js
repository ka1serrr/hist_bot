  ...
  bot.on("message", async (msg) => {
    ...
    try {
      ...
      if ((await user?.current_state) === "first") {
        ...
        console.log(text);
        if (
          text.toLowerCase == "столовая" ||
          text.toLowerCase == "столовка" ||
      ...
      if ((await user?.current_state) === "seventh") {
        if (text.toLowerCase() === "дедлайн" || text.toLowerCase() === "дэдлайн") {
          ...
          return bot.sendPhoto(chatId, "https://ibb.co/mN4bqcs", {
            caption:
              "Узнайте любимых преподавателей в образах дедов Морозов. Ответом станет слово состоящее из 1 буквы фамилии первого «деда», 4 и 9 букв фамилии третьего, 7-ой  второго, и 2 -ой четвертого.",
          });
        }
        return bot.sendMessage(chatId, answers.wrong_answer);
      }
